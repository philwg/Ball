...
char keypressed;                                  // Le caractère lu au clavier
GLint xold, yold, anglex, angley;                 // Les contrôles des éléments gérés à la souris
GLdouble angleScene {5.0}, angleBall {0.0};       // Les angles de rotation des sous-scènes
...
void ballScene()        //-------------------------------- La structure OpenGL de la scène
{
    glPushMatrix();
        ...
        glPushMatrix();
            glRotatef(angleBall, 0.0, 0.0, 1.0);   // Préparer la rotation de la balle
...
void idle()     //---------------------------------------- Définition du rebond comme tâche de fond
{
    if (true) {
        ...
        moveBall();                   // on calcule la position suivante
...
void moveBall()     //------------------------------------ Gestion du déplacement de la Balle
{
